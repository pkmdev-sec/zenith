...
VERSION="latest"
SCOPE="${ZENITH_SKILLS_SCOPE:-user}"
TARGET_DIR="${ZENITH_SKILLS_DIR:-}"
...
download_file() {
  ...
  echo "curl or wget is required to install Zenith skills." >&2
  exit 1
}
...
resolve_target_dir() {
  ...
  case "$SCOPE" in
    repo)
      printf '%s/.agents/skills/zenith\n' "$PWD"
      ;;
    user)
      codex_home="${CODEX_HOME:-$HOME/.codex}"
      printf '%s/skills/zenith\n' "$codex_home"
      ;;
    *)
...
step "Installing Zenith skills ${resolved_version} (${SCOPE})"
...
archive_path="$tmp_dir/zenith-skills.tar.gz"
...
printf 'Zenith skills %s installed successfully.\n' "$resolved_version"
