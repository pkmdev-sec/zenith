...
VERSION="${1:-latest}"
INSTALL_BIN_DIR="${ZENITH_INSTALL_BIN_DIR:-$HOME/.local/bin}"
INSTALL_APP_DIR="${ZENITH_INSTALL_APP_DIR:-$HOME/.local/share/zenith}"
SKIP_PATH_UPDATE="${ZENITH_INSTALL_SKIP_PATH_UPDATE:-0}"
path_action="already"
path_profile=""
...
download_file() {
  ...
  echo "curl or wget is required to install Zenith." >&2
  exit 1
}
...
add_to_path() {
  ...
  profile="${ZENITH_INSTALL_SHELL_PROFILE:-$HOME/.profile}"
  if [ -z "${ZENITH_INSTALL_SHELL_PROFILE:-}" ]; then
    case "${SHELL:-}" in
      */zsh)
        profile="$HOME/.zshrc"
        ;;
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Zenith." >&2
    exit 1
  fi
}

warn_command_conflict() {
  expected_path="$INSTALL_BIN_DIR/zenith"
  resolved_path="$(command -v zenith 2>/dev/null || true)"
  ...
  if [ "$resolved_path" != "$expected_path" ]; then
    step "Warning: current shell resolves zenith to $resolved_path"
    step "Run now: export PATH=\"$INSTALL_BIN_DIR:\$PATH\" && hash -r && zenith"
    step "Or launch directly: $expected_path"

    case "$resolved_path" in
      *"/node_modules/zenith-agent/"* | *"/node_modules/.bin/zenith")
        step "If that path is an old global npm install, remove it with: npm uninstall -g zenith-agent"
        ;;
    esac
  fi
}
...
resolve_release_metadata() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest Zenith release version." >&2
      exit 1
    fi
  else
  ...
  bundle_name="zenith-${resolved_version}-${asset_target}"
  archive_name="${bundle_name}.${archive_extension}"
  download_url="${ZENITH_INSTALL_BASE_URL:-https://github.com/zenith-agent/zenith/releases/download/v${resolved_version}}/${archive_name}"
...
step "Installing Zenith ${resolved_version} for ${asset_target}"
...
Workarounds:
  ...
  - install via pnpm instead: pnpm add -g zenith-agent
  - install via bun instead: bun add -g zenith-agent
EOF
  exit 1
fi
...
step "Linking zenith into $INSTALL_BIN_DIR"
cat >"$INSTALL_BIN_DIR/zenith" <<EOF
#!/bin/sh
set -eu
exec "$INSTALL_APP_DIR/$bundle_name/zenith" "\$@"
EOF
chmod 0755 "$INSTALL_BIN_DIR/zenith"
...
case "$path_action" in
  ...
  *)
    ...
    step "Run: hash -r && zenith"
    ;;
esac

warn_command_conflict

printf 'Zenith %s installed successfully.\n' "$resolved_version"
