...
function Resolve-ReleaseMetadata {
  ...
  $bundleName = "zenith-$resolvedVersion-$AssetTarget"
  $archiveName = "$bundleName.$BundleExtension"
  $baseUrl = if ($env:ZENITH_INSTALL_BASE_URL) { $env:ZENITH_INSTALL_BASE_URL } else { "https://github.com/zenith-agent/zenith/releases/download/v$resolvedVersion" }
...
$installRoot = Join-Path $env:LOCALAPPDATA "Programs\zenith"
$installBinDir = Join-Path $installRoot "bin"
$bundleDir = Join-Path $installRoot $bundleName

$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("zenith-install-" + [System.Guid]::NewGuid().ToString("N"))
...
"@
  ...
  $shimPath = Join-Path $installBinDir "zenith.cmd"
  Write-Host "==> Linking zenith into $installBinDir"
  @"
@echo off
"$bundleDir\zenith.cmd" %*
"@ | Set-Content -Path $shimPath -Encoding ASCII
  ...
  if (-not $alreadyOnPath) {
    ...
    Write-Host "Updated user PATH. Open a new shell to run zenith."
  } else {
    Write-Host "$installBinDir is already on PATH."
  }

  $resolvedCommand = Get-Command zenith -ErrorAction SilentlyContinue
  if ($resolvedCommand -and $resolvedCommand.Source -ne $shimPath) {
    Write-Warning "Current shell resolves zenith to $($resolvedCommand.Source)"
    Write-Host "Run in a new shell, or run: `$env:Path = '$installBinDir;' + `$env:Path"
    Write-Host "Then run: zenith"
    if ($resolvedCommand.Source -like "*node_modules*@companion-ai*zenith*") {
      Write-Host "If that path is an old global npm install, remove it with: npm uninstall -g zenith-agent"
    }
  }

  Write-Host "Zenith $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
