...
function fail(message) {
	console.error(`[zenith] ${message}`);
	process.exit(1);
}

function resolveBundledNodeVersion() {
	const requestedNodeVersion = process.env.ZENITH_BUNDLED_NODE_VERSION?.trim();
	if (requestedNodeVersion) {
		if (compareSemver(parseSemver(requestedNodeVersion), parseSemver(minBundledNodeVersion)) < 0) {
			fail(
				`ZENITH_BUNDLED_NODE_VERSION=${requestedNodeVersion} is below the supported floor ${minBundledNodeVersion}`,
			);
		}
...
function writeLauncher(bundleRoot, target) {
	if (target.launcher === "unix") {
		const launcherPath = resolve(bundleRoot, "zenith");
		writeFileSync(
			launcherPath,
			[
				"#!/bin/sh",
				...
				'exec "$ROOT/node/bin/node" "$ROOT/app/bin/zenith.js" "$@"',
				"",
			].join("\n"),
			"utf8",
		);
	...
	writeFileSync(
		resolve(bundleRoot, "zenith.cmd"),
		[
			"@echo off",
			"setlocal",
			'set "ROOT=%~dp0"',
			'"%ROOT%node\\node.exe" "%ROOT%app\\bin\\zenith.js" %*',
			"",
		].join("\r\n"),
		"utf8",
	);
	writeFileSync(
		resolve(bundleRoot, "zenith.ps1"),
		[
			'$Root = Split-Path -Parent $MyInvocation.MyCommand.Path',
			'& "$Root\\node\\node.exe" "$Root\\app\\bin\\zenith.js" @args',
			"",
		].join("\r\n"),
		"utf8",
	);
...
function validateBundle(bundleRoot, target) {
	...
	run(nodeExecutable, ["-e", "require('./app/.zenith/npm/node_modules/better-sqlite3'); console.log('better-sqlite3 ok')"], {
		cwd: bundleRoot,
	});
}
...
function main() {
	...
	const stagingRoot = mkdtempSync(join(tmpdir(), "zenith-native-"));
	const outDir = resolve(appRoot, "dist", "release");
	const bundleRoot = resolve(stagingRoot, `zenith-${packageJson.version}-${target.id}`);
	...
	const appZenithDir = resolve(appDir, ".zenith");
	extractTarball(resolve(appZenithDir, "runtime-workspace.tgz"), appZenithDir, "-xzf");
	rmSync(resolve(appZenithDir, "runtime-workspace.tgz"), { force: true });
	...
	console.log(`[zenith] native bundle ready: ${archivePath}`);
}
