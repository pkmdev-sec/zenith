...
import { ZENITH_LOGO_HTML } from "../logo.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const appRoot = resolve(here, "..");

const patchResults = [];
function patchReplace(source, search, replacement, label) {
	const patched = source.replace(search, replacement);
	const matched = patched !== source;
	patchResults.push({ label, matched });
	if (!matched) {
		console.warn(`\x1b[33m⚠ Patch did not match: ${label}\x1b[0m`);
	}
	return patched;
}
...
if (!piPackageRoot) {
	console.warn("[zenith] pi-coding-agent not found, skipping Pi patches");
}
...
const workspaceRoot = resolve(appRoot, ".zenith", "npm", "node_modules");
...
const workspaceArchivePath = resolve(appRoot, ".zenith", "runtime-workspace.tgz");
...
function resolvePackageManager() {
	...
	const requested = process.env.ZENITH_PACKAGE_MANAGER?.trim();
...
function installWorkspacePackages(packageSpecs) {
	...
	if (!packageManager) {
		process.stderr.write(
			"[zenith] no supported package manager found; install npm, pnpm, or bun, or set ZENITH_PACKAGE_MANAGER.\n",
		);
		return false;
	}
	...
	if (result.status !== 0) {
		...
		process.stderr.write(`[zenith] ${packageManager} failed while setting up bundled packages.\n`);
		return false;
	}
...
function restorePackagedWorkspace(packageSpecs) {
	...
	mkdirSync(resolve(appRoot, ".zenith"), { recursive: true });

	const result = spawnSync("tar", ["-xzf", workspaceArchivePath, "-C", resolve(appRoot, ".zenith")], {
		stdio: ["ignore", "ignore", "pipe"],
		timeout: 300000,
	});
...
function ensurePackageWorkspace() {
	...
	writeFileSync(
		...
		JSON.stringify({ name: "zenith-packages", private: true }, null, 2) + "\n",
		"utf8",
	);
	...
	const spinner = setInterval(() => {
		...
		process.stderr.write(`\r${frames[frame++ % frames.length]} setting up zenith... ${elapsed}s`);
	}, 80);
	...
	if (!result) {
		...
		process.stderr.write(`\r✓ zenith ready (${elapsed}s)\n`);
	}
}
...
function ensurePandoc() {
	...
	if (process.env.ZENITH_SKIP_PANDOC_INSTALL === "1") return;
	...
	console.log("[zenith] installing pandoc...");
	const result = spawnSync(brewPath, ["install", "pandoc"], {
		stdio: "inherit",
		timeout: 300000,
	});
	if (result.status !== 0) {
		console.warn("[zenith] warning: pandoc install failed, run `zenith --setup-preview` later");
	}
}
...
if (packageJsonPath && existsSync(packageJsonPath)) {
	...
	if (pkg.piConfig?.name !== "zenith" || pkg.piConfig?.configDir !== ".zenith") {
		pkg.piConfig = {
			...(pkg.piConfig || {}),
			name: "zenith",
			configDir: ".zenith",
...
for (const entryPath of [cliPath, bunCliPath].filter(Boolean)) {
	...
	if (cliSource.includes('process.title = "pi";')) {
		cliSource = patchReplace(cliSource, 'process.title = "pi";', 'process.title = "zenith";', 'cli-process-title');
	}
	const stdinErrorGuard = [
		"const zenithHandleStdinError = (error) => {",
		...
		"    }",
		"};",
		'process.stdin?.on?.("error", zenithHandleStdinError);',
	].join("\n");
	if (!cliSource.includes('process.stdin?.on?.("error", zenithHandleStdinError);')) {
...
if (interactiveModePath && existsSync(interactiveModePath)) {
	...
	if (interactiveModeSource.includes("`π - ${sessionName} - ${cwdBasename}`")) {
		let patched = patchReplace(interactiveModeSource, "`π - ${sessionName} - ${cwdBasename}`", "`zenith - ${sessionName} - ${cwdBasename}`", 'window-title-session');
		patched = patchReplace(patched, "`π - ${cwdBasename}`", "`zenith - ${cwdBasename}`", 'window-title-cwd');
		writeFileSync(interactiveModePath, patched, "utf8");
	}
}
...
if (existsSync(sessionSearchIndexerPath)) {
	...
	const replacement =
		'const sessionsDir = process.env.ZENITH_SESSION_DIR ?? process.env.PI_SESSION_DIR ?? path.join(os.homedir(), ".pi", "agent", "sessions");';
...
if (oauthPagePath && existsSync(oauthPagePath)) {
	...
	let changed = false;
	const target = `const LOGO_SVG = \`${ZENITH_LOGO_HTML}\`;`;
...
if (alphaHubAuthPath && existsSync(alphaHubAuthPath)) {
	...
	const bodyAttr = `style="font-family:system-ui,sans-serif;text-align:center;padding-top:20vh;background:#0a1525;color:#c8dcf5"`;
	const logo = `<h1 style="font-family:monospace;font-size:48px;color:#64b4ff;margin:0">zenith</h1>`;
	const newSuccess = `'<html><body ${bodyAttr}>${logo}<h2 style="color:#64b4ff;margin-top:16px">Logged in</h2><p style="color:#8c9cbd">You can close this tab.</p></body></html>'`;
	const newError = `'<html><body ${bodyAttr}>${logo}<h2 style="color:#ef4444;margin-top:16px">Login failed</h2><p style="color:#8c9cbd">You can close this tab.</p></body></html>'`;
...
if (existsSync(piMemoryPath)) {
	...
	const memoryReplacement =
		'const MEMORY_DIR = process.env.ZENITH_MEMORY_DIR ?? process.env.PI_MEMORY_DIR ?? join(homedir(), ".pi", "memory");';
	...
	const execReplacement = [
		'const execBinary = process.env.ZENITH_NODE_EXECUTABLE || process.env.ZENITH_EXECUTABLE || "pi";',
		'      const execArgs = process.env.ZENITH_BIN_PATH',
		'        ? [process.env.ZENITH_BIN_PATH, "--prompt", prompt]',
		'        : ["-p", prompt, "--print"];',
...
const failed = patchResults.filter(p => !p.matched);
if (failed.length > 0) {
	console.warn(`\n\x1b[33m⚠ ${failed.length} patch(es) did not match:\x1b[0m`);
	for (const f of failed) console.warn(`  - ${f.label}`);
	console.warn(`This may indicate an incompatible pi-coding-agent version.`);
	console.warn(`Expected: @mariozechner/pi-coding-agent@0.62.x\n`);
}
