...
Save a checkpoint: `save_checkpoint` with stage='plan'.
...
Save checkpoint with stage='research' listing all research files as artifacts.
...
Before delivery, run the verification gate:
1. `verify_citations` on the output file — fix any FATAL/MAJOR issues
2. `validate_output` with workflowType="deepresearch" — fix any structural issues
3. `export_bibtex` to generate a .bib companion file
4. `save_checkpoint` with stage='deliver'
