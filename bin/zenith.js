...
if (compareNodeVersions(parseNodeVersion(process.versions.node), parseNodeVersion(MIN_NODE_VERSION)) < 0) {
  ...
  console.error(`zenith requires Node.js ${MIN_NODE_VERSION} or later (detected ${process.versions.node}).`);
  ...
  console.error(isWindows
    ? "irm https://zenith.is/install.ps1 | iex"
    : "curl -fsSL https://zenith.is/install | bash");
  process.exit(1);
}
