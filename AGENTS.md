...
Pi subagent behavior does **not** live here. The source of truth for bundled Pi subagents is `.zenith/agents/*.md`, which the runtime syncs into the Pi agent directory. If you need to change how `researcher`, `reviewer`, `writer`, or `verifier` behave, edit the corresponding file in `.zenith/agents/` instead of duplicating those prompts here.

## Pi subagents

Zenith ships four bundled research subagents:
...
They are defined in `.zenith/agents/` and invoked via the Pi `subagent` tool.
