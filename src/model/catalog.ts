...
export function buildModelStatusSnapshotFromRecords(
	...
	if (available.length === 0) {
		...
		guidance.push(
			"Run `zenith model login <provider>` (OAuth) or configure an API key (env var, auth.json, or models.json for custom providers).",
		);
		guidance.push("After auth is in place, rerun `zenith model list` or `zenith setup model`.");
	} else if (!current) {
		guidance.push(`No default research model is set. Recommended: ${recommended?.spec}.`);
		guidance.push("Run `zenith model set <provider/model>` or `zenith setup model`.");
