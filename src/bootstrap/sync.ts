...
export function syncBundledAssets(appRoot: string, agentDir: string): BootstrapSyncResult {
	...
	syncManagedFiles(resolve(appRoot, ".zenith", "themes"), resolve(agentDir, "themes"), "themes", state, result);
	syncManagedFiles(resolve(appRoot, ".zenith", "agents"), resolve(agentDir, "agents"), "agents", state, result);
