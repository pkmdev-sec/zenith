...
Run research code inside Docker containers while Zenith stays on the host. The container gets the project files, runs the commands, and results sync back.
...
```bash
docker build -t zenith-experiment .
docker run --rm -v "$(pwd)/results":/workspace/results zenith-experiment
```
